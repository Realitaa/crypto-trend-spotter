<script setup lang="ts">
    ...
    const inflectionPoints = computed(() =>
      (props.inflections || []).map(p => [p.time * 1000, p.value])
    )
        ...
        series: [
          {
            ...
            lineStyle: {
              width: 2,
              type: "dashed",
              color: params => (params.value[1] >= 0 ? "#a855f7" : "#ef4444")
            }
          },
    ...
    onMounted(async () => {
      ...
      watch(() => props.isHydrated, async h => {
        if (h && !chartInstance) await mountChart()
      })
    })
